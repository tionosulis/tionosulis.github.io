...
**Server review flow (wajib — baik draft baru maupun existing):**
1. Tulis post → `draft: true` → commit & push
2. Start Eleventy server (`localhost:8080`), berikan link draft ke user
3. User review langsung di browser — jika ada revisi, perbaiki (server auto-reload)
4. User stop server → kill Eleventy process
5. Tunggu instruksi selanjutnya — jangan commit/push tanpa persetujuan eksplisit dari user

**Publish flow:**
6. User perintahkan publish → set `draft: false` → commit & push ke main
...
## 5. SVG Hero Images & OG Pipeline

### Dimensi

Semua hero SVG baru: **1200×630** (1.91:1 — standar Open Graph).

### Wajib: Konversi ke OG PNG

Setiap post dengan `image` di frontmatter (hero SVG) WAJIB memiliki file PNG
di `content/assets/img/og/`. Generate dengan Sharp:

```bash
node -e "require('sharp')('content/assets/img/<hero>.svg').resize(1200,630).png().toFile('content/assets/img/og/<post-slug>.png')"
```

Kalau SVG sudah 1200×630, cukup tanpa resize:
```bash
node -e "require('sharp')('content/assets/img/<hero>.svg').png().toFile('content/assets/img/og/<post-slug>.png')"
```

Verifikasi file benar-benar ada di `content/assets/img/og/` sebelum commit.
...
- [ ] Server review flow done — draft dicek langsung di localhost
...
- [ ] File PNG di `content/assets/img/og/` benar-benar ada
